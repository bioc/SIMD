...
writefile=NULL,reportfile=NULL,mreratio=3/7, psd=2, mkadded=1,f=1)
    ...
    if(length(datafile)==2) {
        ...
        #data1<-read.table(datafile[1], header=TRUE,sep="\t", as.is=TRUE)
        #data2<-read.table(datafile[2], header=TRUE,sep="\t", as.is=TRUE)
        data1<-datafile[1]
        data2<-datafile[2]
        data3<-data1
        data4<-data2
    ...
    #data1<-read.table(datafile[1], header=TRUE,sep="\t",as.is=TRUE)
    #data2<-read.table(datafile[2], header=TRUE,sep="\t", as.is=TRUE)
    #data3<-read.table(datafile[3], header=TRUE,sep="\t", as.is=TRUE)
    #data4<-read.table(datafile[4], header=TRUE,sep="\t", as.is=TRUE)
    ...
    if(is.null(cpgfile)) {
        ...
        #cpg<-read.table(cpgfile, header=TRUE,sep="\t",as.is=TRUE)
        cpg<-cpgfile
        print(cpg[1,])
    }
    if(is.null(mrecpgfile)) {
        ...
        #mrecpg<-read.table(mrecpgfile, header=TRUE,sep="\t",as.is=TRUE)
        mrecpg<-mrecpgfile
        print(mrecpg[1,])
    }
    cpg[,4] = cpg[,4]/f
    ...
    length(smsm1[,1]),pvalue=rep(0,length(smsm1[,1])))
    ...
    colnames(cpgpq)=c("chr", "chrSt","chrEnd","Medip1","Medip2","MRE1","MRE2",
    "cg","mrecg","pvalue",'Ts')
